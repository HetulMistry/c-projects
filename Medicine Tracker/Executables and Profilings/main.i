...
void displayMedication(const struct Medication *med)
{
  printf("Medication: %s\n", med->name);
  printf("Dosage: %d mg\n", med->dosage);
  printf("Next Dose Time: %s", ctime(&(med->nextDoseTime)));
}
# 64 "main.c"
void updateNextDoseTime(struct Medication *med, int interval)
{
  med->nextDoseTime += interval;
}
# 77 "main.c"
...
# 130 "main.c"
    ...
    {
      remindUser(&med1);
      updateNextDoseTime(&med1, 30);
    }

    if (currentTime >= med2.nextDoseTime)
    {
      remindUser(&med2);
      updateNextDoseTime(&med2, 40);
    }
  }
